...
public class JoinNodesMapperTest {
	...
	public void setUp() throws Exception {
		driver = new MapDriver<LongWritable, Text, Text, Text>(new JoinNodesMapper());
	}

	@Test
	public void shouldOutputAsNode() throws Exception {
		output = driver.withInputKey(new LongWritable(0)).withInputValue(new Text("0	A	Aname")).run();

		assertThat(output.size(), is(1));

		assertThat(output.get(0).getFirst(), equalTo(new Text("NA")));
		assertThat(output.get(0).getSecond(), equalTo(new Text("0	A	Aname")));
