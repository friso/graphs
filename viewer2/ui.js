...
function addQuery(query) {
	handleResult(query, model.addQueryResult)
}

function replaceQuery(query) {
	handleResult(query, model.populateFromQueryResult)
}
...
function handleResult(query, method) {
	writeLog('Running query...')
	restApi.getCypherResult(query, function(result, err) {
		if (err) {
			handleError(err)
		} else {
			writeLog('Got response...')
			if (result.length == 0) {
				writeLog('Query returned empty set!')
